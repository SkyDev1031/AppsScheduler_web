...
import { support_confTransferApi } from '../../api/OriginAPI.js';
import { DEF_IMAGE, _ERROR_CODES } from "../../config";
import { crypto_path, toast_error, toast_success, upload_path } from "../../utils";
import { GlobalContext } from '../../contexts';

export default function SendBox({ sendMessage, uploadFile, editMessage, setEditMessage, moderatorId, seller_id }) {
    ...
    const { sellerId, settings, user } = useContext(GlobalContext);
    ...
    const send = () => {
        ...
        if(moderatorId){
            const data = {
                ScreenNamePhoneEmail: user.username,
                cid: 1,
                TransferAmount: settings.amountPerAns,
                TransferFee: settings.amountToNet,
                TransferFeeType: 'Expedited',
                seller_id: seller_id,
                product_id: sellerId.id
            };
        
            var res = support_confTransferApi(data)
